  ...
  const tabButton = (id: AcademyTab, label: string, Icon: typeof Dumbbell, badge?: number) => (
    ...
    >
      ...
      {badge !== undefined && badge > 0 && tab !== id && (
        <span className="absolute -top-1 -right-1 inline-flex items-center justify-center w-5 h-5 rounded-full bg-eldritch/20 text-eldritch text-xs font-bold border border-eldritch/30">
          {badge}
        </span>
      )}
    </button>
