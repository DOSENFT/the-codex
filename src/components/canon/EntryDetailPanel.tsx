  ...
  return (
    <div data-entry-detail={detail.title} className="border-t border-bronze/20">
      ...
      {/* ══ BAND 3 · HOW TO USE IT ═════════════════════════════════════════
             Advice, split at the source's own capitalised headings. Empty is
             honest: nobody has a note for this and the app will not invent one.

             THE SUBHEAD IS NOT DECORATION — it names whose voice the bullets are
             in, off `tacticsSource`. Until slice 9b it said "Canon's own words"
             unconditionally, which was true only because every bullet was canon's.
             `feature-notes.ts` broke that, so the line has to read the field: the
             alternative is the app telling him canon said something the app made
             up. `bands.test.ts` asserts the two can never disagree. */}
      {detail.bands.tactics.length > 0 && (
        <section data-band="3" className="px-4 pb-5">
          ...
          <p className="mt-2 text-[10px] leading-relaxed text-forge-2">
            {detail.bands.tacticsSource === 'house'
